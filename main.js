...
(function() {
  ...
  window.fetch = function(...args) {
    ...
    // Skip middleware for auth endpoints to allow proper response handling
    const isAuthEndpoint = typeof url === 'string' && (
      url.includes('/api/registration/') || 
      url.includes('/api/login')
    );
    
    if (isAuthEndpoint) {
      // For auth endpoints, don't intercept - let the handler deal with response
